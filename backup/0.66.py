...
class App(customtkinter.CTk):
        def __init__(self):
                ...
                self.bottoneupdate=customtkinter.CTkButton(master=self.frame_bikinibottom, text="Aggiorna",width=425)
        ...
        def add_value(self):
                f = open("filtri.txt", "a")
                f.write("\n%s,%s,%s,%s,%s,%s" %(self.textbox1.get(),self.textbox2.get(),self.textbox3.get(),self.textbox4.get(),self.textbox5.get(),self.textbox6.get()))
                f.close()
                self.table.add_row("")
