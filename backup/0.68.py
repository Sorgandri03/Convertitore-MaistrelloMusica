...
class App(customtkinter.CTk):
        def __init__(self):
                ...
                self.frame_bottom.grid(row=2, column=1,padx=20,pady=20)
                ...
                self.table = CTkTable(master=self.frame_top, column=6, values=heading)
                ...
                self.bottoneupdate=customtkinter.CTkButton(master=self.frame_bikinibottom, text="Aggiorna",width=425,command=self.aggiorna)
                self.bottoneupdate.grid(row=0,column=1)
                self.bottoneaggiungi=customtkinter.CTkButton(master=self.frame_bikinibottom, text="Aggiungi filtro",width=425,command=self.add_value)
                self.bottoneaggiungi.grid(row=0,column=4)
        ...
        def convertitore_ibs(self):
                ...
                for i, row in df.iterrows():
                        ...
                        df.at[i,'prezzov'] = (round(float(row[28])*percentuale)-0.01)
                        if row[33]==0:
                ...
                for filtroattuale in filtriattuali:
                        ...
                        prezzo.append(filtroattuale[5].removesuffix("\n"))

                for i, row in df.iterrows():
                        for c in codice:
                                if c==df.at[i,'codart']:
                                        indice=codice.index(c)
                                        df.at[i,'prezzov'] = (round(float(prezzo[indice])*percentuale)-0.01)
